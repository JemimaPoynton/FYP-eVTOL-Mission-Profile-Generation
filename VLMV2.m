function [lift, CL, CD, CM, distribution, AC] = VLMV2(lift, mesh, airflow, N, plotBool, xref, Sref, Cref)
    ...
    for n = 1:N(2) % panel i position in y
        ...
        for ji = 1:N(1) % i index of panel j (vortex influencing panel mn (i))
            for jj = 1:N(2) % j index of panel j
                ...
                storeB(ji,jj,:) = squeeze(B(m,n,:))';
                storepc(ji,jj,:) = squeeze(pc(m,n,:))';
...
for n = 1:N(2)
    for m = 1:N(1)
        ...
        cd_mn(m,n) = cp(m,n)*sab*uVec; % perpendicular (pressure) force * resolve drag components
        cl_mn(m,n) = norm((cp(m,n))*uVec' - cd_mn(m,n)*sab); % magnitude of non-dimensional perpendicular (pressure) force - drag component

        x = [mesh.x(1,2) mesh.x(2,2) mesh.x(2,1) mesh.x(1,1)];
        y = [mesh.y(1,2) mesh.y(2,2) mesh.y(2,1) mesh.y(1,1)];
% 
%         area = polyarea(x,y);
%         b(m,n) = area/c_mn(m,n);
        b(m,n) = mesh.y(m,n+1) - mesh.y(m,n);

        dL = 1.225*circ(m,n)*b(m,n)*airflow.U^2;

        CL = CL + (1/Sref)*b(m,n)*cl_mn(m,n);
        CD = CD + (1/Sref)*b(m,n)*cd_mn(m,n);
