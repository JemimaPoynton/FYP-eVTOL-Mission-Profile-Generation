function [lift, CL, CD, CM, distribution, AC] = VLM(lift, airflow, N, plotBool, xref)
...
%% Handle symmetry
if length(lift) > 1
    mesh = combineWings(lift(1), lift(2)); %combined mesh
    ...
    for n = 1:N(2) % panel i position in y
        ...
        for ji = 1:N(1) % i index of panel j (vortex influencing panel mn (i))
            for jj = 1:N(2) % j index of panel j
                ...
                % Bound vortex
                r0_BV = squeeze(B(m,n,:) - A(m,n,:))';
                r1_BV = squeeze(pc(m,n,:) - A(m,n,:))';
                r2_BV = squeeze(pc(m,n,:) - B(m,n,:))';

                V(1,1:3) = inducedV_1Vortex(r0_BV, r1_BV, r2_BV);

                % Right Trailing vortex
                r0_RT = squeeze(C(m,n,:) - B(m,n,:))';
                r1_RT = r2_BV;
                r2_RT = squeeze(pc(m,n,:) - C(m,n,:))';

                V(2,1:3) = inducedV_1Vortex(r0_RT, r1_RT, r2_RT);

                % Free Stream Vortex
                r0_FV = squeeze(D(m,n,:) - C(m,n,:))';
                r1_FV = r2_RT;
                r2_FV = squeeze(pc(m,n,:) - D(m,n,:))';
 
                V(3,1:3) = inducedV_1Vortex(r0_FV, r1_FV, r2_FV);

                % Left Trailing Vortex
                r0_LT = squeeze(A(m,n,:) - D(m,n,:))';
                r1_LT = r2_FV;
                r2_LT = squeeze(pc(m,n,:) - A(m,n,:))';
...
for n = 1:N(2)
    for m = 1:N(1)
        ...
        cp = 2*circ(m,n)/airflow.U;

        % sc: local to panel mn
        cd_mn(m,n) = (cp/c_mn(m,n))*sab*uVec; % non-dimensional perpendicular (pressure) force * resolve drag components
        cl_mn(m,n) = norm((cp/c_mn(m,n))*uVec' - cd_mn(m,n)*sab); % magnitude of non-dimensional perpendicular (pressure) force - drag component
...
for j = 1:N(2)
    ...
    if control.yc(1,j) > 1 && length(lift) > 1
        c_ = getChord(lift(2), control.yc(1,j)); % full chord at span position n
    else
        c_ = getChord(lift(1), control.yc(1,j));
    end
...
if length(lift) > 1
    c_ = (2*getChord(lift(1),0) + getChord(lift(1),lift(1).span) + getChord(lift(2),lift(2).span))/4; % mean aerodynamic chord
else
    c_ = (getChord(lift(1),0) + getChord(lift(1),lift(1).span))/2;
end

CM = M*(1/(Sref*c_)); % correcting non-dimensionalisation

AC = (CM/CL)*c_; % distance of aerodynamic centre from LE
